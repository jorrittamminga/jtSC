		...
		*/
		...
		n.do{|i|
			...
			if (par[0][0] < startTimeTr, {
				...
				dataa=[0,0,0,1,par[0][0]];//freq, amp, dur, attackTime, startTime, freqMin, freqMax
				dataa[2]=partials[i*2][3]-partials[i*2][2];//dur
				if (dataa[2]>durTr, {
					//par.postln;
					dataa[0]=par[1].mean;//freq
					dataa[1]=par[2].maxItem.ampdb;//amp
