/*
w=Window.new.front; w.addFlowLayout; e=EZSlider2D(w, 200@200);

*/

EZSlider2D : EZGui {

	var <slider2D, <hiBox,<loBox, <unitView, <>controlSpec
	,<lo, <hi, popUp=false, numSize,numberWidth,unitWidth;
	var <>round = 0.001;
	...
	*new { arg parent, bounds, label, controlSpec, action, initVal,
	...
	init { arg parentView, bounds, label, argControlSpec, argAction, initVal,
		...
		slider2D = GUI.slider2D.new(view, rangerBounds);//rangeslider
		...
		slider2D.action_({ |sl|
				this.lo_(controlSpec.map(sl.x));
				this.hi_(controlSpec.map(sl.y));
				this.doAction;
			});
		...
		slider2D.receiveDragHandler = { arg slider;
			slider.valueAction = controlSpec.unmap(GUI.view.currentDrag);
		};

		slider2D.beginDragAction = { arg slider;
	...
	lo_ { |val|
		...
		slider2D.x_(controlSpec.unmap(lo));
	}

	hi_ { |val|
		hi = controlSpec.constrain(val);
		hiBox.value_(hi.round(round));
		slider2D.y_(controlSpec.unmap(hi));
	}
	setColors{arg stringBackground, stringColor, sliderColor,  numBackground,numStringColor,
		...
		sliderColor.notNil.if{
				slider2D.background_(sliderColor);};
